...
class CosDoubleWell(Model):
    """ 
    Class implementing the force and potential for a double well potential of
    the form U(q) = a * cos(q) + b/2 * (q-c)^2
    """
    def __init__(self, dim=1, a=1.0,b=1.0, c=0.0):
    
        super(CosDoubleWell, self).__init__(dim)
        self.a = a
        self.b = b
        self.c = c
        
    def comp_force(self, q):
        return self.a * np.sin(q) - self.b * (q-self.c)
  
    def comp_potential(self, q):
        return self.a * np.cos(q) + .5 * self.b * (q-self.c)**2
